                                                ...
                                                <tr data-status="{{ $claim['status'] ?? 'pending' }}"
                                                    ...
                                                    <td>
                                                        <div class="d-flex flex-column flex-sm-row gap-1">
                                                            ...
                                                            {{-- ===== EMAIL LOG BUTTON (DESKTOP) ===== --}}
                                                            @if (isset($claim['document_no']))
                                                                <button type="button"
                                                                    class="btn btn-sm text-white email-log-btn"
                                                                    style="display:inline-block; min-width:10px; text-align:center; background-color: #6f42c1; border: none;"
                                                                    data-bs-toggle="modal" data-bs-target="#emailLogModal"
                                                                    data-doc="{{ $claim['document_no'] }}"
                                                                    title="View Email Logs">
                                                                    <i class="bi bi-envelope-open"></i>
                                                                    <span class="badge bg-light text-dark ms-1 email-log-count" data-doc="{{ $claim['document_no'] }}">…</span>
                                                                </button>
                                                            @endif
                                                            ...
                                                            <div class="col-12 mt-2">
                                                                <div class="d-flex gap-2 flex-wrap">
                                                                    @if (isset($claim['document_no']))
                                                                        ...
                                                                        {{-- ===== EMAIL LOG BUTTON (MOBILE) ===== --}}
                                                                        <button type="button"
                                                                            class="btn btn-sm text-white email-log-btn flex-fill"
        ...
        <table class="table table-hover table-striped align-middle text-center rounded-3 overflow-hidden mb-0">
            <thead class="bg-light text-secondary">
                <tr>
                    ...
                    <th class="py-3">💬 Admin Approval Remarks</th>
...
Thank you for your cooperation.</textarea>
    ...
    <div class="modal fade" id="emailLogModal" tabindex="-1" aria-labelledby="emailLogModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-centered modal-fullscreen-sm-down">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 18px;">
                <div class="modal-header text-white border-0"
                    style="background: linear-gradient(135deg, #007bff 0%, #0056b3 100%); border-radius: 18px 18px 0 0;">
                    <h5 class="modal-title fw-semibold fs-5" id="emailLogModalLabel">
                        <i class="bi bi-envelope-open me-2"></i>
                        Email Logs : <span id="emailLogDocNo" class="text-white fw-bold"></span>
                    </h5>
                ...
                <div class="modal-body p-3">
                    <!-- Spinner -->
                    <div id="emailLogSpinner" class="text-center py-5">
                        <div class="spinner-border text-purple" role="status" style="color: #6f42c1; width: 2.5rem; height: 2.5rem;"></div>
                        <p class="mt-3 text-muted small">Loading email logs...</p>
                    </div>

                    <!-- Table -->
                    <div class="table-responsive" id="emailLogTableWrap" style="display:none;">
                        <table class="table table-hover table-striped align-middle mb-0" id="emailLogDataTable" style="font-size:13px;">
                            <thead style="background: linear-gradient(135deg, #007bff 0%, #0056b3 100%);">
                                
                                    <th class="py-3 text-dark text-center" style="width:45px;">ID</th>
                                    <th class="py-3 text-dark text-center" style="min-width:150px;">Document</th>
                                    <th class="py-3 text-dark text-center" style="min-width:175px;">Sender</th>
                                    <th class="py-3 text-dark text-center" style="min-width:175px;">Receiver</th>
                                    <th class="py-3 text-dark text-start" style="min-width:220px;">Subject</th>
                                    <th class="py-3 text-dark text-start" style="min-width:250px;">Message Body</th>
                                    <th class="py-3 text-dark text-center" style="min-width:90px;">Route</th>
                                    <th class="py-3 text-dark text-center" style="min-width:150px;">Date &amp; Time</th>
                                
                            </thead>
                            <tbody id="emailLogTableBody"></tbody>
                        
                    </div>

                    <!-- Empty state -->
                    <div id="emailLogEmpty" class="text-center py-5" style="display:none;">
                        <i class="bi bi-inbox fs-1 text-secondary opacity-50 d-block mb-2"></i>
                        <p class="text-muted">No email logs found for this document.</p>
                    </div>

                    <!-- Error -->
                    <div id="emailLogError" class="alert alert-danger mt-3 d-none">
                        <i class="bi bi-exclamation-triangle-fill me-2"></i> Error loading email logs. Please try again.
                    </div>
                </div>
    ...
    <style>
        ...
        /* Email log button */
        .email-log-btn {
            background: linear-gradient(135deg, #6f42c1 0%, #4b2d8a 100%) !important;
            border: none !important;
            transition: all 0.2s ease;
        }

        .email-log-btn:hover {
            background: linear-gradient(135deg, #4b2d8a 0%, #3a2068 100%) !important;
            transform: translateY(-1px);
            box-shadow: 0 4px 10px rgba(111, 66, 193, 0.4);
        }

        /* Email log modal table */
        #emailLogDataTable thead th {
            font-size: 12px;
            font-weight: 600;
        }

        #emailLogDataTable tbody td {
            font-size: 12px;
            vertical-align: middle;
        }
        
        /* Document number white text */
        #emailLogDataTable tbody td:nth-child(2) {
            color: #0066cc!important;
            font-weight: 600;
        }
        
        /* Email body preview styling */
        .email-body-preview {
            cursor: help;
        ...
        .email-log-type-reminder {
            background-color: #fff3cd;
            color: #856404;
        }

        .email-log-type-status {
        ...
        .email-log-type-revision {
            background-color: #f8d7da;
            color: #721c24;
        }

        .email-log-type-approval {
            background-color: #d4edda;
            color: #155724;
        }

        .email-log-type-upload {
    ...
    <script>
        $(document).ready(function() {
            ...
            var emailLogCache = {}; 
            var emailLogsFetched = false;
            var emailLogsFetching = false;
            var emailLogsFetchCallbacks = [];

            function fetchAllEmailLogs(callback) {
                if (emailLogsFetched) {
                    if (callback) callback();
                    return;
                }
                if (emailLogsFetching) {
                    if (callback) emailLogsFetchCallbacks.push(callback);
                    return;
                }

                emailLogsFetching = true;
                if (callback) emailLogsFetchCallbacks.push(callback);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        var allLogs = response.all_logs || [];
                        
                        // Filter to only show client route logs
                        var clientLogs = allLogs.filter(function(log) {
                            return log.route === 'client';
                        });
                        
                        // Group by uw_doc using filtered logs
                        clientLogs.forEach(function(log) {
                            var doc = log.uw_doc;
                            if (!doc) return;
                            if (!emailLogCache[doc]) emailLogCache[doc] = [];
                            emailLogCache[doc].push(log);
                        });
                        
                        emailLogsFetched = true;
                        emailLogsFetching = false;
                        emailLogsFetchCallbacks.forEach(function(cb) { cb(); });
                        emailLogsFetchCallbacks = [];
                    },
                    error: function() {
                        emailLogsFetched = true;
                        emailLogsFetching = false;
            ...
            // Update all count badges using cached data
            function loadEmailLogCounts() {
                fetchAllEmailLogs(function() {
                    $('.email-log-count').each(function() {
                        var doc = $(this).data('doc');
                        if (!doc) return;
                        var logs = emailLogCache[doc] || [];
                        var count = logs.length;
                        $(this).text(count);
                        if (count > 0) {
                            $(this).removeClass('bg-light text-dark').addClass('bg-warning text-dark');
                        } else {
                            $(this).removeClass('bg-warning text-dark').addClass('bg-light text-dark');
                        }
                    });
                });
            }
                ...
                table = $('#claimsTable').DataTable({
                    ...
                    initComplete: function() {
                        ...
                        setTimeout(function() {
                            ...
                            // Load email log counts after table is ready
                            loadEmailLogCounts();
                        }, 100);
                    }
                });
            ...
            function updateAppointmentStats() {
                var today = new Date();
                var counts = {
                    "7": 0, "15": 0, "30": 0, "60": 0, "90": 0, "90plus": 0, "all": 0
                };
            ...
            function filterTableByDate(selectedDays) {
                ...
                table.draw();
                return table.rows({ filter: 'applied' }).count();
            }
            ...
            function initializeTooltips() {
                document.querySelectorAll('#viewReportModal [data-bs-toggle="tooltip"]')
                    .forEach(function(el) {
            ...
            $('#viewReportModal').on('show.bs.modal', function(event) {
                ...
                tbody.html(' <td colspan="4">Loading...</td> ');
                errorBox.hide();
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (!data || data.length === 0) {
                            tbody.html(' <td colspan="4">No documents uploaded yet.</td> ');
                        } else {
                            data.forEach(function(doc) {
                                var appRemCell;
                                if (doc.app_rem && doc.app_rem !== '-') {
                                    if (doc.app_rem_tooltip) {
                                        appRemCell = '<span ' +
                                            'data-bs-toggle="tooltip" ' +
                                            'data-bs-placement="top" ' +
                                            'title="' + doc.app_rem_tooltip.replace(/"/g, '&quot;').replace(/\n/g, '&#10;') + '" ' +
                                            'style="cursor:help; border-bottom:1px dotted #999;">' +
                                            doc.app_rem +
                                            '</span>';
                                    } else {
                                        appRemCell = '<span>' + doc.app_rem + '</span>';
                                    }
                                } else {
                                    appRemCell = '-';
                                }
                                ...
                                tbody.append(
                                    ...
                                    '<td class="py-2">' + escapeHtml(doc.remarks || '-') + '</td>' +
                                    '<td class="py-2">' + appRemCell + '</td>' +
                                    '<td class="py-2">' +
                                        '<div class="d-flex gap-2 justify-content-center">' +
                                            '<a href="' + doc.url + '" target="_blank" class="btn btn-sm text-white rounded-pill" style="background-color: #000088;">' +
                                                '<i class="bi bi-box-arrow-up-right me-1"></i>Open' +
                                            '</a>' +
                                            '<a href="' + doc.url + '" download class="btn btn-sm text-white rounded-pill" style="background-color: #28A745;">' +
                            ...
                            document.querySelectorAll('#documentTableBody [data-bs-toggle="tooltip"]')
                                .forEach(function(el) {
                                    new bootstrap.Tooltip(el, { trigger: 'hover' });
                                });
                        }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $message.removeClass('alert-danger')
                            ...
                            .html('<i class="bi bi-check-circle me-2"></i>Reminder sent successfully!')
                            .show();
            ...
            // ===== EMAIL LOG MODAL HANDLER =====
            $('#emailLogModal').on('show.bs.modal', function(event) {
                var button = $(event.relatedTarget);
                var docNo = button.data('doc');

                $('#emailLogDocNo').text(docNo);
                $('#emailLogSpinner').show();
                $('#emailLogTableWrap').hide();
                $('#emailLogEmpty').hide();
                $('#emailLogError').addClass('d-none');
                $('#emailLogTableBody').empty();

                // Use the single cached fetch — no per-doc AJAX call needed
                fetchAllEmailLogs(function() {
                    $('#emailLogSpinner').hide();

                    var logs = emailLogCache[docNo] || [];
                    ...
                    logs = logs.slice().sort(function(a, b) {
                        return new Date(b.curdatetime) - new Date(a.curdatetime);
                    });

                    var tbody = $('#emailLogTableBody');
                    tbody.empty();

                    logs.forEach(function(log, index) {
                        // Determine badge class by route
                        var badgeClass = 'email-log-type-email';
                        var routeLower = (log.route || '').toLowerCase();
                        var subLower   = (log.sub   || '').toLowerCase();

                        if (routeLower.includes('reminder') || subLower.includes('reminder')) {
                            badgeClass = 'email-log-type-reminder';
                        } else if (routeLower.includes('revision') || subLower.includes('revision')) {
                            badgeClass = 'email-log-type-revision';
                        } else if (routeLower.includes('approval') || subLower.includes('approval')) {
                            badgeClass = 'email-log-type-approval';
                        } else if (routeLower.includes('upload') || subLower.includes('upload')) {
                            badgeClass = 'email-log-type-upload';
                        } else if (routeLower.includes('status') || subLower.includes('status')) {
                            badgeClass = 'email-log-type-status';
                        }
                        ...
                        if (log.curdatetime) {
                            try {
                                dateStr = new Date(log.curdatetime).toLocaleString('en-US', {
                                    year: 'numeric', month: 'short', day: 'numeric',
                                    hour: '2-digit', minute: '2-digit'
                                });
                            } catch(e) { dateStr = log.curdatetime; }
                        }

                        // Get full message body and create truncated version with tooltip
                        var fullBody = log.body || '-';
                        var truncatedBody = fullBody;
                        if (fullBody.length > 80) {
                            truncatedBody = fullBody.substring(0, 80) + '…';
                        }
                        
                        // Escape HTML for safety
                        var escapedFullBody = escapeHtml(fullBody);
                        var escapedTruncatedBody = escapeHtml(truncatedBody);
                        
                        // Create body cell with Bootstrap tooltip
                        var bodyCell = '';
                        if (fullBody !== '-') {
                            bodyCell = '<span class="email-body-preview" data-bs-toggle="tooltip" data-bs-placement="top" title="' + escapedFullBody.replace(/"/g, '&quot;') + '" style="cursor:help; border-bottom:1px dotted #999;">' + escapedTruncatedBody + '</span>';
                        } else {
                            bodyCell = '-';
                        }

                        var senderDisplay   = escapeHtml(log.sender   || 'N/A');
                        var receiverDisplay = escapeHtml(log.receiver  || 'N/A');

                        tbody.append(
                            ' <tr>' +
                            '<td class="text-center fw-bold text-muted">' + (index + 1) + '</td>' +
                            '<td class="text-center fw-semibold" style="color:#ffffff;">' + escapeHtml(log.uw_doc || 'N/A') + '</td>' +
                            '<td class="text-center small">' + senderDisplay + '</td>' +
                            '<td class="text-center small">' + receiverDisplay + '</td>' +
                            '<td class="text-start small">' + escapeHtml(log.sub || 'N/A') + '</td>' +
                            '<td class="text-start small">' + bodyCell + '</td>' +
                            '<td class="text-center"><span class="email-log-type-badge ' + badgeClass + '">' + escapeHtml(log.route || 'N/A') + '</span></td>' +
                            '<td class="text-center text-nowrap small">' + dateStr + '</td>' +
                            '</tr>'
                        );
                    });

                    // Initialize tooltips for the new body cells
                    setTimeout(function() {
                        $('#emailLogTableBody [data-bs-toggle="tooltip"]').each(function() {
                            new bootstrap.Tooltip(this, { trigger: 'hover' });
                        });
                    }, 100);

                    $('#emailLogTableWrap').show();

                    // Refresh count badge for this doc
                    var count = logs.length;
                    $('.email-log-count[data-doc="' + docNo + '"]').each(function() {
                        $(this).text(count);
                        if (count > 0) {
                            $(this).removeClass('bg-light text-dark').addClass('bg-warning text-dark');
                        } else {
            ...
            if (!isMobile) {
                ...
                // On mobile, load counts after a short delay
                setTimeout(function() {
                    loadEmailLogCounts();
                }, 800);
            }

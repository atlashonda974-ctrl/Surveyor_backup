...
public function getFiles(Request $request)
{
    try {
        ...
        // Build API URL
        $apiUrl = "http://172.16.22.204/dashboardApi/clm/getSurvData.php?surv=All&zone={$zone}";
        ...
        foreach ($allApiData as $docNo => $apiData) {
            ...
            // If still no activity, check created_at from latest file
...
public function markAsInProcess(Request $request)
{
    try {
        ...
        // Get the LATEST record to check report type
        ...
        $reportType = $latestFile->rep_tag; // P/R or F/R
        
        // Update ALL records of the SAME report type for this document
        $updatedCount = FileTab::where('doc_no', $docNo)
            ->where('rep_tag', $reportType)
            ->update([
                'plr_final' => $action,
    ...
    public function storeSurveyor(Request $request)
{
    try {
        ...
        // Send welcome email with credentials - passing false for isResend
    ...
    private function generateRandomPassword($length = 12)
    {
        ...
        $password .= '!@#$%*&'[rand(0, 6)]; // special char
...
{
    try {
        // Find the surveyor - using User model with role = 'Surveyor'
        ...
        // Update the surveyor's password
        $surveyor->password = Hash::make($tempPassword);
        $surveyor->save();
        
        // Resend the welcome email - passing true for isResend
